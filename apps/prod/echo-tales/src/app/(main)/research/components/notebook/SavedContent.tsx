...
import type { SavedContent as SavedContentType, SavedContentType as ContentType } from '@/types/notebook';
...
export function SavedContent({ content, onContentChange }: SavedContentProps) {
  ...
  const filters: { value: ContentType | 'all'; label: string }[] = [
    ...
    { value: 'approach', label: 'Approaches' },
    { value: 'topic', label: 'Topics' },
  ];

  const handleUnsave = async (item: SavedContentType) => {
    try {
      ...
      console.error('[SavedContent] Error unsaving:', err);
    }
  };
    ...
    return (
      ...
      >
        ...
        <p className="text-sm text-white/45 font-light max-w-md mx-auto leading-relaxed">
          Save Reddit posts, YouTube videos, and research approaches to revisit them later.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-1.5 overflow-x-auto">
        {filters.map((f) => {
          const count = f.value === 'all' ? content.length : content.filter((c) => c.content_type === f.value).length;
      ...
      {filteredContent.length > 0 ? (
        ...
        <div className="text-center py-8 text-sm text-white/40 font-light">
          No {filter === 'all' ? '' : filter} content saved.
        </div>
      )}
    </div>
  );
...
function SavedContentCard({
  ...
  const label = getTypeLabel(item.content_type);
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        ...
        {/* Title */}
        <h4 className="text-sm font-light text-white/90 line-clamp-2 mb-2 leading-snug">{item.title}</h4>

        {/* Description */}
        {item.description && (
          <p className="text-[13px] leading-relaxed text-white/45 font-light line-clamp-2 mb-2">{item.description}</p>
        )}
        ...
        <div className="flex items-center justify-between text-[10px] font-mono tracking-wider uppercase text-white/30">
          ...
          {item.metadata?.subreddit && (
            <span className="text-purple-300/60">r/{item.metadata.subreddit}</span>
          )}
          {item.metadata?.channel_name && (
            <span className="text-purple-300/60 truncate max-w-[120px]">{item.metadata.channel_name}</span>
          )}
        </div>
      </div>
