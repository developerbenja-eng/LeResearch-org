...
import { Image as ImageIcon, ZoomIn, ExternalLink, Sparkles, Loader2 } from 'lucide-react';
import type { ReaderFigure } from '@/types/reader';

interface FigurePopoverContentProps {
  figure: ReaderFigure;
  paperId: string;
  onViewFullSize?: () => void;
}

export function FigurePopoverContent({ figure, paperId, onViewFullSize }: FigurePopoverContentProps) {
  const [isGeneratingAI, setIsGeneratingAI] = useState(false);
  const [aiDescription, setAiDescription] = useState(figure.ai_description);
      ...
      {figure.image_url && !imageError ? (
        <div className="relative w-full h-40 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden group">
          <Image
            ...
            alt={figure.caption || figure.figure_name}
            fill
      ...
      {aiDescription ? (
        <div className="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-2">
          <div className="flex items-center gap-1 mb-1">
            ...
            <span className="text-xs font-medium text-purple-600 dark:text-purple-400">AI Analysis</span>
          </div>
